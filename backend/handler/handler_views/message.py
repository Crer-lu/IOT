...
def create(request):
    ...
    try:
        ...
        # set online status
        device.status = True
        device.last_activate_time = timezone.now()
        device.save()
        # create message
        message = Message.objects.create(device=device, info=dict['info'], latitude=dict['latitude'], longitude=dict['longitude'], value=dict['value'], alert=dict['alert'])
...
def info(request):
    ...
    try:
        ...
        for message in messages:
            data.append({'messageid':message.id, 'deviceid': message.device.id, 'info': message.info, 'latitude': message.latitude, 'longitude': message.longitude, 'alert':message.alert,'time_stamp': message.time_stamp, 'value': message.value})
...
def all(request):
    ...
    if verify_jwt_token(uid, token.split(' ')[1]):
        ...
        data = []
        for message in messages:
            data.append({'messageid':message.id, 'deviceid': message.device.id, 'devicekind': message.device.kind, 'info': message.info, 'latitude': message.latitude, 'longitude': message.longitude, 'time_stamp': message.time_stamp, 'value': message.value, 'alert':message.alert})
        return JsonResponse({'status': 200, 'message': '查询成功', 'data': data})
    else:
