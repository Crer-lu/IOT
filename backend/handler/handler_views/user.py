...
def register(request):
    ...
    try:
        user = User.objects.create(name=dict['name'], password=dict['password'], email=dict['email'], phone=dict['phone'])
...
def query_info(request):
    ...
    if verify_jwt_token(userid, token.split(' ')[1]):
        user = User.objects.get(id=userid)
        return JsonResponse({'status': 200, 'message': '查询成功', 'data': {'id':user.id, 'name': user.name, 'email': user.email, 'phone': user.phone}})
